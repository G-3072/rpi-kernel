...
void uart_init(void){
    ...
    mmio_write(UART_ICR, 0x7FF);   // clears all intterupts

    uart_setBaudrate(115200);

    mmio_write(UART_LCRH, (1<<4)|(1<<5)|(1<<6));    // set to 8bit transmit and enable FIFO transmit buffer

    mmio_write(UART_IMSC, (1<<1)|(1<<4)|(1<<5)|(1<<6)|(1<<7)|(1<<8)|(1<<9)|(1<<10));   // enabele interruppts

    mmio_write(UART_CR, (1 << 0) | (1 << 8) | (1 << 9));   // turn UART back on
}
